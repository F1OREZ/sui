...
fn crash_recovery() {
    ...
    cps.handle_internal_batch(4, &[(1, t1), (2, t2), (3, t3)], &_committee)
        .unwrap();
    ...
    cps.handle_internal_batch(7, &[(4, t4), (5, t5), (6, t6)], &_committee)
        .unwrap();
...
fn latest_proposal() {
    ...
    assert!(cps1
        .handle_internal_set_checkpoint(summary.clone(), &transactions)
        .is_err());
...
fn checkpoint_integration() {
    ...
    while cps.get_locals().next_checkpoint < 10 {
        ...
        cps.handle_internal_batch(next_tx_num, &some_fresh_transactions[..], &_committee)
            .unwrap();

        // If we have a previous checkpoint, now lets try to process again?
        if let Some((summary, transactions)) = checkpoint_opt.take() {
            assert!(cps
                .handle_internal_set_checkpoint(summary, &transactions)
                .is_ok());
        ...
        let initial_proposal = cps.set_proposal().unwrap();
        ...
        let latest_proposal = cps.set_proposal().unwrap();
